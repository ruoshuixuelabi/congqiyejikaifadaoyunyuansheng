...
//        boolean isAnyMale = peopleStream.anyMatch(Person::isMale);
    ...
    private static void collectionCollectDemo() {
        List<String> streamToList = Stream.of("wyf", "www", "foo", "bar").collect(Collectors.toList());
        Set<String> streamToSet = Stream.of("wyf", "wyf", "foo", "bar").collect(Collectors.toSet());
        List<String> streamToCollection = Stream.of("wyf", "www", "foo", "bar").collect(Collectors.toCollection(ArrayList::new));
    }

    private static void stringCollectDemo() {
        String streamToString = Stream.of("wyf", "www", "foo", "bar").collect(Collectors.joining(",", "[", "]"));
        String streamToStringOnlyDelimiter = Stream.of("wyf", "www", "foo", "bar").collect(Collectors.joining(","));
...
//                .collect(collectingAndThen(toList(),List::size));
    ...
    private static void groupingByToMap() {
        //按照特定功能处理分组，接收一个Function接口。
        Map<Integer, List<String>> groupingByMap = Stream.of("wyf", "www", "foobar", "barfoo")
                .collect(Collectors.groupingBy(String::length));
        //partitioningBy是一种特殊的 groupingBy方法，它接收Predicate作为参数，只将数据分成false和true两组。
